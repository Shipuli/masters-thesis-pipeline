...
With machine learning methods, we mean algorithms and statistical models that derive underlying principles from data using patterns and inference \cite{bzdok}.

Both of these methods are trying to beat the efficient market hypothesis (EMH).
EMH states that the all the current public information available should already be seen in the price of the stock \cite{day}.
...
Both supervised \cite{serez} and unsupervised learning techniques \cite{adresic} have been tried to predict the prices but recently neural networks especially have gained a lot of interest due to their adaptablity to any non-linear data.
...
Although MFF is seemingly the most popular, better results in price predictions have been able to achieve with convolutional neural networks (CNN) and long short-term memory (LSTM) \cite{senguptaa}.
With convolutional neural networks, the upper hand to regular MFF seems to be the ability to express same amount of complexity with smaller amount neurons, although no direct comparisons have been made with these two \cite{senguptaa}.
...
As stock data is literally a time series, the LSTM's ability to remember previous states seems to separate it with other methods \cite{senguptaa}.
...
For this section, we examined 24 different stock data analysis pipelines that handle or are able to handle big data. \cite{wang} \cite{day} \cite{nam} \cite{balan} \cite{islam} \cite{le} \cite{li} \cite{umadevi} \cite{chen} \cite{peng} \cite{gu} \cite{chungho} \cite{das} \cite{adresic} \cite{serez} \cite{serez2} \cite{zhanga} \cite{wenga} \cite{yu} \cite{saha} \cite{curtis} \cite{palmer} \cite{snively} \cite{juan}
...
This partial data usually consists of historical end of day data, which is the aggregated statisctics of the stocks after the market has closed for the day.
...
The most common method for ingesting stock market data of the ones that did report their ingestion was found to be custom scripts. \cite{nam} \cite{le} \cite{li} \cite{chen} \cite{adresic} \cite{serez} \cite{wenga} \cite{saha}
...
No pipeline reported a usage of mixing cloud platfrom products with open-source products.
...
With pipelines that analyze data with machine learning methods, the quality of data can be somewhat important \cite{sessions}.
...
When we researched existing pipelines we noticed that machine learning libraries such as Tensorflow and Theano are still quite used even when data can scale to enormous amounts \cite{islam} \cite{li} \cite{le}.
...
The Elasticsearch Logstash Kibana (ELK) stack for monitoring the data flow inside the system and individual components and the ModelDB to specifically monitor the Machine learning models that are produced by the pipeline.
...
In this stack, Logstash, an open-source data ingestion pipeline tool, is used to ingest the log data.
...
However, a pure ELK stack is still very popular option to handle log data \cite{elastic}.
...
The library is said to have second version coming up, but at the time of writing this the library has not had major update for an year \cite{modeldb}.
...
On top of tools for the managing explained in ModelDB paragraph, MLFlow puts more emphasis on packaging the models and the deployment of these models into actual use \cite{mlflow}.
...
MLFlow is as project quite young having its first full version released in June 2019, but it has a healthy development community and is actively developed \cite{mlflow}.
